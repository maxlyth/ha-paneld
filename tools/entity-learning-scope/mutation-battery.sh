#!/usr/bin/env bash
# Mutation battery for the entity-learning scope lane (pins, exclusions, view scoping).
#
...
#
#   bash tools/entity-learning-scope/mutation-battery.sh [name-substring]
set -euo pipefail
...
mutations = json.load(open(os.path.join(work, "tools/entity-learning-scope/mutations.json")))
results = []
    ...
    try:
        if m["suite"] == "kotlin":
            ...
            res = run(
                ...
                f"CHROME={chrome} node --test --test-name-pattern='[Cc]ustom|dashboard picker fits|unknown dashboard' browser-behavior.test.mjs",
...
if infra:
    ...
    for name, _, _ in infra:
        print(f"  bash tools/entity-learning-scope/mutation-battery.sh {name}")
