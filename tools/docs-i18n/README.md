...
`plan` writes a new manifest only when the canonical target does not already exist. The manifest contains hashes and structural commitments, not a second copy of the source prose. `export-plan` reconstructs its exact masked packet payload for a separately managed file outside the public repository. `apply` accepts already reviewed, manifest-bound packet results and creates one locale output plus its receipt atomically without overwriting existing files. `validate` performs the read-only CI checks over the exact manifest, source, receipts, links, fragments and localized output trees.
