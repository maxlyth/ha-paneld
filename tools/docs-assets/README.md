...
Why images are hosted rather than committed is covered in [`docs/infrastructure.md`](../../docs/infrastructure.md). The short version: git history here is effectively append-only, so a binary added to the repository stays in it permanently, and product photography carries licensing the project cannot verify. Serving it from project-owned storage keeps that content out of the repository while leaving a copy the project fully controls.

## Requirements

`python3` with [Pillow](https://pypi.org/project/Pillow/) for image handling, and [rclone](https://rclone.org/) configured with an R2 remote for publishing. Reading published images needs nothing — the bucket is public.

Originals and published bytes are read from and written to `$HA_PANELD_DOCS_ASSETS_DIR`, which defaults to `.docs-assets/` beside the checkout and is ignored by git. Keep that store somewhere durable and backed up: it is the only copy of the untouched source images, and a published object can never be replaced.
...
`verify` fetches every published object over HTTPS and confirms the served bytes match the manifest hash, the content type is correct, and the immutable cache header is present. Run it periodically rather than only at publish time — a hash mismatch on an object that was already published means the served content was substituted, which the manifest is the only thing positioned to detect.
