#!/usr/bin/env bash
# Mutation battery for the Auto-resolved learning scope lane.
#
...
#
#   bash tools/auto-resolved-scope/mutation-battery.sh [name-substring]
set -euo pipefail
...
mutations = json.load(open(os.path.join(work, "tools/auto-resolved-scope/mutations.json")))
results = []
...
if infra:
    ...
    for name, _, _ in infra:
        print(f"  bash tools/auto-resolved-scope/mutation-battery.sh {name}")
