...
- **Navigate: reload on the same URL** — the navigate control (`text.<panel>_navigate`) drives **local** HA paths (scheme + host are stripped; `homeassistant://navigate/<path>` deeplink). Today, navigating to the path the panel is **already showing** is a frontend no-op — which reads as "it ignored me," especially when the target equals the panel's default dashboard. Make a repeat of the current path **reload** instead: compare the requested path to the last-commanded value — different → navigate; same → trigger the existing reload action. (The panel can't observe the Companion's live route, so the compare uses *last-commanded* as a heuristic.)
...
- **Navbar "reload page" icon** — an extra navbar soft-key that **reloads the current dashboard page**, shown/active only when the **HA Companion is the frontmost app** (foreground-package gated, so it never reloads another app). One tap to recover a stale or half-rendered view without a full app restart. Candidate mechanisms: inject the frontend's pull-to-refresh via a root `input swipe` (keeps the current route), or fall back to the hard reload + return-to-intended-dashboard above.
- **Slim admin launcher + home-app management** — panel vendors ship a bloated launcher (notably **Sonoff 4.x+**) that we never want as the home app. ha-paneld should make the **HA Companion the home/default** so a panel boots straight to the dashboard (no vendor launcher, no boot-to-launcher detour), and provide its **own ultra-slim launcher** for admin — an app grid + admin shortcuts (Android Settings, ha-paneld config, developer options) — reachable **on demand** (the navbar **Launcher** button, or a corner gesture) rather than as the day-to-day home. With root, sideline the vendor launcher (`pm` set-home / disable) so it can't grab Home. Replaces today's behaviour where the Launcher button opens the *device's* launcher (= the vendor crapware). Two impl routes: a built-in `CATEGORY_HOME` activity in ha-paneld, or ship/install the slim `l.l` launcher via the installer (see below). Pairs with the per-panel home-dashboard / boot-to-dashboard work above.
