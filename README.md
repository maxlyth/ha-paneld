...
- **Re-announce discovery on HA restart** — subscribe to `homeassistant/status` and re-publish MQTT discovery on the `online` (birth) message. Discovery is currently published with `retain=true` so Home Assistant can rebuild the panel's entities after a restart, but retained discovery configs make entities **sticky** — a deleted entity is resurrected on the next restart unless its retained topic is cleared. Re-announcing on birth lets discovery drop `retain` (or makes it harmless), so deletions stick and a renamed/decommissioned panel doesn't orphan entities. Pairs with a teardown that clears retained discovery **and** state for a panel (and the old id on rename).
