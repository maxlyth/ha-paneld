...
**Latest release — 0.9.1:** a round of fixes and refinements to the 0.9.0 **built-in dashboard renderer** — pull-to-refresh now only fires from a deliberate pull at the very top edge (not from scrolling a view), an **App Configuration** entry in the Home Assistant sidebar opens the panel's own settings, and dashboard sizing matches the Companion app, with a nudge toward the panel display-density control for crisper scaling. Also: LED effects stop reliably and animate more smoothly, the soft navbar no longer swallows taps at the bottom of the screen, an opt-in System WebView auto-update keeps the panel's pinned build current, and TPA10-class panels move to LineageOS WebView 150 so camera streams autoplay. Full notes for every release are in [CHANGELOG.md](CHANGELOG.md).
