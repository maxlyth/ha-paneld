...
**Latest release — 0.9.0:** the headline is the **built-in dashboard renderer** — ha-paneld can now render the Home Assistant dashboard itself (experimental, off by default), turning a panel into a single-app appliance: switch the picker and it borrows an installed Companion's sign-in automatically, or provision a fresh panel with zero typing from your admin machine, with page-level resilience built for weeks-long uptimes. 0.9 also brought light/dark theming across the web UI, app and dashboard, LED effects, MQTT over TLS, and a self-refreshing Configure page. Full notes for every release are in [CHANGELOG.md](CHANGELOG.md).
