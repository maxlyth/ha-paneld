...
> [!IMPORTANT]
> **The relay sysfs class was renamed between firmware versions.** Only the **initial** image (1.0.2) uses `/sys/class/st_relay`; **all newer** images (1.1.0+) use `/sys/class/strelay` — reporter-confirmed in [#3](https://github.com/maxlyth/ha-paneld/issues/3) and matched by the firmware diff. ha-paneld probes both class names and uses whichever the panel exposes — see the relay section below.

Downloads (Smatek, as shared by the reporter in [#3](https://github.com/maxlyth/ha-paneld/issues/3)):

- `S9_1920x1200_20240712_Android_US` — **1.0.2** *(original Smatek link `docs.smatek.store:10001` is unreachable; contact Smatek or the reporter in #3 for a copy)*
...
> [!TIP]
> The firmware also carries an **RGB status LED** (`led_r` / `led_g` / `led_b`), a **vibrator/haptic** motor and **Ethernet-activity LEDs** — none currently exposed by ha-paneld.
...
- Smatek S9E stock firmware (shared by the reporter in #3): 1.0.2 / 20240712 and 1.1.0 / 20251202 — the two images analysed. **Original Smatek download links (`docs.smatek.store:10001`) are no longer reachable**; contact Smatek support or the issue reporter for a current download link.
