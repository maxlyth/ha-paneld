...
| `hardware.camera` | Optional; default absent | Boolean, and its absence is a third state rather than `false`. Omit it and the panel offers the camera whenever Android enumerates one, which is why a board with a working camera needs no profile entry at all. Set `true` to declare a camera the panel must offer even if enumeration fails, and `false` to suppress one Android reports but that is not a usable room camera, such as an HDMI capture input. |
