...
- [`community-lenovo-thinksmart-view-lineageos.yaml`](community-lenovo-thinksmart-view-lineageos.yaml): a draft for one contributor's Lenovo ThinkSmart View reflashed with LineageOS 8.1. It selects Android sleep because brightness zero remains visibly lit, records that touch does not wake the panel, and notes the contributor's confirmation that Home Assistant can dim it, turn it fully off and wake it again. See the [profile-specific evidence notes](lenovo-thinksmart-view-lineageos.md).
- [`community-rpi4-konstakang-lineageos.yaml`](community-rpi4-konstakang-lineageos.yaml) — a draft for one contributor's Raspberry Pi 4 Model B running KonstaKANG's LineageOS Android 16 build with a separately sourced HDMI touchscreen. This is an assembly rather than a retail panel, so the attached display's density and wake behaviour are the owner's to establish; the profile's one declared behaviour is the `keyevent` screen route, for a board that exposes no Linux backlight device. Whether a touch wakes the sleeping panel depends on that touchscreen. On the reported unit it needed an Android input device configuration that enables touch wake, installed through a Magisk module because its directory is read-only. See the [profile-specific evidence notes](raspberry-pi-4-konstakang.md).
...
- [`community-yc-sm10p.yaml`](community-yc-sm10p.yaml) — a draft for one contributor's Portworld YC-SM10P on Android 11 userdebug. It records the verified `su-blpower` screen-off route and no user LED; the declared EM3071x light/proximity device-tree nodes do not answer on the I2C bus, so those sensors remain absent. See the [profile-specific evidence notes](yc-sm10p.md).
