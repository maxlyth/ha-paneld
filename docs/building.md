...
You don't need to build ha-paneld to use it — `scripts/install.sh` fetches a signed release (see the
[README](../README.md#install)). This page is for contributors and forkers. For the VS Code
devcontainer workflow and signing local release builds for in-place fleet updates, see also
[local-builds.md](local-builds.md).
