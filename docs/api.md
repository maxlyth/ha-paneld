...
| `/api/v1/config/home-dashboards`, `/api/v1/config/ha-area`, `/api/v1/config/probe-broker`, `/api/v1/config/probe-log-sink` | Home dashboard and area choices plus MQTT-broker and log-sink preflights used by guided setup and Configure. The log-sink probe transmits one marked record without changing saved configuration; verify the returned marker at a syslog collector because a completed socket write is not delivery acknowledgement. |
...
| `/api/v1/logship/status` | Passive, uncached log-shipping summary for the Configure card. It reports whether shipping is enabled and configured, with a redacted human-readable transport state and last failure; polling it transmits no records. |
