> [!IMPORTANT]
> Dieses Dokument wurde maschinell erstellt und automatisch gegengeprüft, jedoch nicht systematisch von Personen geprüft, die diese Sprache sprechen. Die englische Dokumentation ist maßgeblich. [Englisches Original lesen](../tts.md) oder [ein Issue zur Übersetzungskorrektur öffnen](https://github.com/maxlyth/ha-paneld/issues/new?template=translation_correction.yml).

# Sprachausgabe (TTS) aus Home Assistant

Speak any Home Assistant TTS voice (Piper, Home Assistant Cloud, …) on a panel with two `rest_command`s and a small script: HA renders the phrase to an audio URL and the panel downloads and plays it via `POST /play`. Copy the three blocks under [Setup](#einrichtung), then call `script.ha_paneld_say`.

Serverseitige TTS bietet Ihnen moderne Stimmen — deutlich besser als die geräteinterne TTS auf älteren Panels.

> [!NOTE]
> Home Assistant verfügt über **keine MQTT-`media_player`-Plattform**. Daher ist ein Panel (noch) kein `media_player`, an den Sie `tts.speak` direkt richten können — dies steht auf der [Roadmap](../roadmap.md#longer-term-stretch-goals). Diese Anleitung dient als Übergangslösung.
...
Aus einer Automatisierung oder der UI:
