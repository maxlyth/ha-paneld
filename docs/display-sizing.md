...
Android's `wm density` output calls the factory/base logical value `Physical density`. That label is misleading: it is the density Android uses for layout after an override is reset, not a measurement of the display's physical pixels per inch. ha-paneld reports it as the **factory base** and shows physical PPI only for panel profiles whose screen dimensions have been validated independently.
