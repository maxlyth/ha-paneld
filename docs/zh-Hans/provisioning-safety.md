> [!IMPORTANT]
> 本文档由机器生成并经过自动交叉核验，但尚未由中文使用者进行系统审阅。英文文档为权威版本。[阅读英文原文](../provisioning-safety.md)，或[创建翻译更正议题](https://github.com/maxlyth/ha-paneld/issues/new?template=translation_correction.yml)。

# 配置安全与恢复

本页说明 ha-paneld 安装程序和设备群更新程序所采用的检查机制及恢复边界。大多数用户只需使用[配置与设备群更新](provisioning.md)中的命令。这里的详细信息面向需要判断无人值守更新对其面板是否足够安全，或诊断安装程序中止原因的用户。
