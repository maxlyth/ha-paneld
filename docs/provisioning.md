...
Before every upgrade the installer also copies the panel's database — `ha-paneld.db` and its `-wal` and `-shm` sidecars — into the same owner-only directory, mode 600, next to the settings export. That file is the canonical store: configuration, the entity catalog, proximity and ambient history and the on-panel revision history all live in it, and a normal `adb install -r` preserves it. The copy matters when recovery needs an uninstall, which destroys it. Copying it needs a root route on the panel, so it is best-effort: a sandboxed panel prints that only settings could be saved, and the upgrade continues either way.
