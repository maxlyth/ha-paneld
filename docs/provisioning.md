...
Use `--prerelease` before `--provision` to follow the newest published release, including release candidates; a newer stable release still wins. Without it, the installer follows stable releases only. Other routine provisioning options include `--force`, `--builtin`, `--ha-url`, `--ha-token-file`, `--ha-user` and `--ha-pass-file`.
