...
- **ha-paneld and the Companion app now appear in Home Assistant as update entities.** Each panel publishes `update.<panel>_ha_paneld` and `update.<panel>_ha_companion` over MQTT with the installed version, the latest release on the panel's update channel, a link to its release notes and an in-progress state while the panel installs. Installing from Home Assistant installs exactly the release the entity shows, only when it is newer than what is installed, through the same signer-checked path as the Install page, and asks for approval on the panel in Hardened mode. The Companion entity also covers a panel with no Companion app, where it offers to install the minimal app, and a full app from the Play Store, which it shows without offering to update. A panel whose Companion is above its profile's safety cap shows the installed version as unsupported and offers the capped release. The existing Update ha-paneld and Update Companion app buttons remain for this release and will be removed in the next one, so move automations that press them to `update.install`. The buttons now respond only to Home Assistant's own press; an MQTT message to their topics with any other payload is ignored. A panel already added to the Panel Assistant integration keeps that integration's ha-paneld update entity and does not publish a second one.

### Changed

- **The camera indicator now settles down during a camera session you meant to leave running.** It still opens as it always did, a red arc under the lens flashing about once a second at full brightness, because a camera opening when you did not expect it should be impossible to miss. Over the following couple of minutes the gap between flashes stretches to about a minute and the light dims. The flash itself never gets shorter or fades away, so a settled session still tells the room unmistakably, just less often. Nothing about it is configurable and no setting can switch it off. A new camera session, a session reopening after a permission or encoder failure, or the display coming back on all return it to a flash every second; attaching or dropping a viewer within one running session does not. The status LED while the screen is off, the rule that the camera cannot open unless the indicator can be drawn, and the few seconds it stays lit after the camera closes are all unchanged.
...
- **Sandboxed panels without helper-backed hardware now get root-helper guidance.** On a panel where the app cannot use `su` and the profile declares no helper-backed LED, screen, sensor or button, such as the Shelly Wall Display, the provisioning plan said nothing about the root helper, although display sizing, CPU governor, screenshots and performance readings all depend on it there. The plan now recommends it. The installer is unchanged and still installs the helper wherever a root route works.

- **Updating from Git Bash on Windows now finds Android Build-Tools, and a missing tool is reported as one.** Updating a panel that already runs ha-paneld needs `apksigner` and either `aapt` or `aapt2` from Android SDK Build-Tools. The Windows SDK ships `apksigner` as `apksigner.bat`, which the installer could not find under Git Bash, so a computer that had the tools was refused as though it had none, and the refusal pointed at the APK and the panel database rather than the computer. The installer now finds `.bat` and `.cmd` tools under Git Bash, including through an `ANDROID_HOME` set the Windows way. When a tool really is missing or cannot run, the update stops with a message that names every such tool and says how to fix it: where to get Build-Tools and how to point the installer at them, or that `apksigner` needs Java. It no longer blames the APK or the panel database. An APK signed by a different key than the installed app is still refused, now with its own message and the steps for changing keys.
