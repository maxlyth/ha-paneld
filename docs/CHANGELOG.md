...
- **The camera indicator now settles down during a camera session you meant to leave running.** It still opens the way it always did - a red arc under the lens, flashing about once a second at full brightness - because a camera opening when you did not expect it should be impossible to miss. Over the following couple of minutes it backs off, and the two halves of the flash are treated differently on purpose: the flash keeps its half-second length and stays bright, while the gap after it stretches out to about a minute and fades away. A long-running session therefore shows you a clear, unmistakable flash roughly once a minute instead of a light pulsing away on the dashboard all afternoon. What the flash never does is get shorter or fade out - those are fixed, and there is no setting anywhere that turns the indicator off, speeds it up or dims it further. Anything that breaks the continuity puts it back to a full second-by-second flash and starts the clock again: a new camera session, a session reopening after a permission or encoder failure, or the display coming back on after the screen was off. Attaching or dropping a viewer while one session keeps running does not, because a light that flared every time somebody opened a card would be worse than the one this replaces. The off-screen indication on the status LED, the rule that the camera cannot open unless the indicator can be drawn, and the few seconds the indicator stays lit after the camera closes are all unchanged.
