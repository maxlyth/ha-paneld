...
object CompanionRestore {
    ...
    interface Executor {
        ...
        fun commit(plan: Plan, target: TargetInfo): Boolean
    ...
    fun execute(plan: Plan, executor: Executor): Result {
        ...
        if (!executor.commit(plan, target)) {
            executor.discard(plan)
            val relaunched = executor.relaunch(plan.packageName)
            return Result(false, "failed to commit Companion files; rollback attempted", committedFiles = null, relaunched = relaunched)
        }
        val relaunched = executor.relaunch(plan.packageName)
        if (!relaunched) return Result(false, "files restored but Companion relaunch failed", plan.files.size)
        return Result(true, "restored Companion ${plan.packageName}", plan.files.size, relaunched = true)
    }
}
