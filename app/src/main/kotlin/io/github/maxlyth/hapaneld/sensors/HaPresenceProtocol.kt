...
internal object HaPresenceProtocol {
     ...
     * person may deliberately point the panel at a different room than its HA device sits in, such as
     * when the device's own area has no motion entities and a neighbouring room supplies the presence
     * sources. An unknown or blank name falls back to the device's own registry area, so a renamed area
