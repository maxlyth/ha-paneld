...
static constexpr char LOG_TAG[] = "ha-paneld/mww-frontend";

// Configuration matching the ESPHome micro_wake_word component
...
namespace {
    ...
    // Scale factor converting the uint16 MicroFrontend output to float (1/256 * 10), as in
    // microwakeword/inference.py of the OHF-Voice micro-wake-word trainer.
...
size_t MicroFrontendWrapper::processSamples(const int16_t* samples, size_t numSamples, std::vector<float>& out) {
    if (!initialized_) {
    ...
    while (samplesProcessed < numSamples) {
        ...
        struct FrontendOutput output = FrontendProcessSamples(
            &state_, samples + samplesProcessed, numSamples - samplesProcessed, &numSamplesRead);
        if (numSamplesRead == 0) {
            break;
        }
        ...
        if (output.values != nullptr && output.size > 0) {
            for (size_t index = 0; index < PREPROCESSOR_FEATURE_SIZE; index++) {
                const float value = index < output.size ? static_cast<float>(output.values[index]) : 0.0f;
                out.push_back(value * FLOAT32_SCALE);
            }
            ++frames;
        }
    }
