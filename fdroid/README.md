...
2. Confirm the `ANDROID_KEYSTORE_BASE64` / `ANDROID_KEYSTORE_PASSWORD` / `ANDROID_KEY_ALIAS` /
   `ANDROID_KEY_PASSWORD` secrets exist (they already do — `release.yml` uses them).
3. Trigger once via **Actions → F-Droid repo → Run workflow** (or cut a full release).
4. From the run log (or the published landing page), copy the **add-repo URL**:
   `https://maxlyth.github.io/ha-paneld/fdroid/repo?fingerprint=<FP>`
