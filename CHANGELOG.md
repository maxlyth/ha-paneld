...
- **Experimental: a panel with a camera can serve a live stream and a snapshot to Home Assistant, off by default and on trial.** On a device profile that declares a camera, Configure gains a **Camera** card with a switch and four settings — **Resolution**, **Frame rate**, **Bitrate** and an **Exposure** bias in stops, for a camera that reads a room darker or brighter than it looks. Those are what a stream gets when its URL asks for nothing, and a stream URL can override any of them in either direction. Several viewers share one encode session — the first to connect sets it — so a second viewer costs packetisation and network rather than a second encode. The switch's description links the stream and snapshot addresses so they can be opened or copied straight into Home Assistant rather than retyped. With the switch on, the panel serves its camera as a video-only H.264 stream at `rtsp://<panel>:8554/live`, which Home Assistant's Generic Camera takes as a stream source and turns into a WebRTC card, and which go2rtc and Frigate pull directly; `GET /api/v1/camera/snapshot.jpg` returns a JPEG the same integration can poll as a still image. The stream URL can override the configured defaults with `?res=`, `?fps=` and `?kbps=`; the first viewer's choice sets the one encode session and later viewers join it, because a second encode is exactly the cost these panels cannot afford. The camera opens only while a client is actually connected or asking for a frame and closes again afterwards, so an idle panel spends nothing on it. Whenever the camera is open the panel shows a red light directly beneath the camera — an arc curving under the lens at the top centre of the screen, pulsing once a second, which page content cannot cover — on every Android version, and if that light cannot be shown the camera stays closed; while the screen is off the status LED carries the indication instead. It pulses between two visible levels rather than blinking off, so there is no moment when the camera is open and nothing on screen says so. `GET /api/v1/status` gains a `camera` object — including the stream's clients, encoder and the frame rate and bitrate it actually delivered — and the diagnostics dump a `[camera]` line, so a camera that has stopped working is visible rather than silent. The two supported panels that carry a camera, the Tuya TPA10 and the Electron WF1589T, are the only ones where any of this appears; every other panel omits the switch entirely and is unchanged. The app asks for the Android camera permission only after the switch is on, and the camera trial remains video-only and does not use microphone access. In Home Assistant the panel now publishes a **Camera** switch, off by default and exposed the same way as every other optional setting entity, and an image entity showing a still from the camera. The image entity carries a link rather than the picture itself, so Home Assistant fetches a frame only when somebody actually looks at the card and no picture is ever left sitting on the broker; while the switch is off it reads as unavailable rather than showing an old frame, and it is a still rather than a live view, which is what the stream is for. Turning the switch on from Home Assistant arms the trial and nothing more, because Android still withholds the camera until the permission is granted at the panel itself. The panel's own info page gains a **Camera** row carrying the current state and the stream address, beside the reminder not to point this panel's dashboard at its own camera.
### Added

- **The panel now also measures the network path itself, with ICMP echoes, and reports what it finds.** The existing measurement rides the Home Assistant WebSocket, which means it also contains the time Home Assistant takes to answer and the time the panel takes to get round to reading the reply — useful, but not a fact about your network. The panel now additionally sends a small burst of echo requests to the address its Home Assistant connection is actually using, which is a measurement of the path and nothing else. It is deliberately intermittent: the interval between bursts widens as the path keeps proving itself reliable, up to ten minutes, and snaps straight back to thirty seconds the moment a burst looks wrong, so a settled panel spends a handful of packets an hour on this. It needs no root and opens no extra connection. Where a panel will not allow it, or where your Home Assistant host does not answer ICMP at all, the panel says so plainly and carries on using the measurement it already had — it will never report an unanswered echo as packet loss until it has seen at least one answered, because a host that simply filters ICMP is not a broken network. For this release the echo results appear in `/api/v1/diag` and `/api/v1/status` for diagnosis only; they do not raise the on-panel warning yet.

- **The panel now measures the connection to Home Assistant, and tells apart a network that is losing packets from a server that is answering slowly.** A poorly placed access point or a congested 2.4 GHz channel can add seconds to every tap while Android still reports a perfectly good Wi-Fi connection, and until now the only evidence was a dashboard that felt broken and a panel that got the blame. The app already keeps one authenticated Home Assistant WebSocket open whenever it has a reason to (the built-in renderer watching for a Home Assistant restart, a Home Assistant ambient light or presence entity, or a registry watch); it now sends a tiny ping on that socket every ten seconds and measures the round trip on the path your dashboard actually uses. There is no ICMP ping, no background process and no root involved. Over a rolling five-minute window the panel keeps the median, the 95th percentile, the worst round trip, the jitter and how many probes went unanswered. **The prominent warning is reserved for evidence that packets are actually going missing** — a warning when more than 5% of the window's probes are lost, and severe when two in a row go unanswered. How long Home Assistant takes to reply is reported separately, as an **HA response time** row and a response-time likely cause, because a slow reply on a connection that is not dropping anything is a busy server or a busy panel, not a bad network: on a wired panel the round trip can read hundreds of milliseconds while the actual path to the server is under two. Nothing at all is judged for the first three minutes after the app starts, since a panel that is still loading its dashboard is slow for reasons that have nothing to do with the network. One lone slow reply or one lone miss is shown as evidence but never raises an alarm, and any notice ages out by itself as the bad samples leave the window. Home Assistant closing the connection, refusing it while it restarts, rejecting the sign-in or returning an error is counted separately and never reported as packet loss, so a restarting server does not get blamed on your Wi-Fi. A ping that goes unanswered while Home Assistant is still sending anything at all, including a late reply to an earlier ping, is treated the same way: the server is busy, the socket is plainly alive, and the panel keeps it open rather than reconnecting. The Dashboard tab shows a warning at the top of the page in the same style as its other severe warnings, and adds **HA network path** and **HA response time** rows to Runtime diagnostics. On the panel itself a small **HA network unreliable** chip appears in a corner of the dashboard while the warning stands; it turns red when the verdict is severe, never covers the dashboard, and passes every touch straight through. The `/api/v1/diag` report and `/api/v1/status` carry the same verdicts as terse numbers only, never the Home Assistant address, the network name or individual samples. The connection is measured only while the panel actually holds a signed-in Home Assistant connection, so a panel that is still connecting, one that has no credentials, or one whose connection has given up reports it as not measured rather than as healthy.
- **The panel says when Home Assistant is overriding its Dashboard theme.** A Dark or Light choice on the panel yields to a Light or Dark chosen explicitly in Home Assistant, because changing that would mean changing a setting shared with the user's other devices. Until now that was silent: the panel looked healthy and simply did not do what it was told. The Runtime card now notes it, and `GET /api/v1/status` reports `theme_overridden` under `renderer`, alongside the configured policy and the scheme actually on screen, with the fix named: set that Home Assistant user to Auto, or give the panel its own user.
- **The panel can now insist on a dark or light dashboard.** Configure → Built-in renderer gains **Dashboard theme**, with Follow Home Assistant, Dark and Light. Until now the panel only supplied a *default*, which a theme already saved in Home Assistant quietly won; on a kiosk dashboard with the sidebar hidden there was no way to reach the setting that decided it, so the answer was to install a second app. Dark and Light now decide it on the panel, on every Android version, including the ones where the panel previously had no say at all. Follow Home Assistant is the default and behaves exactly as before. Forcing a theme changes only the light/dark part of the choice: a named theme and its colours are untouched, going back to Follow returns that part to the value it had, and the theme saved against your Home Assistant account is never written, so a panel set to Dark cannot darken your phone. A user who has explicitly picked Light or Dark in Home Assistant, rather than Auto, still wins. [Issue #61](https://github.com/maxlyth/ha-paneld/issues/61)

- **A panel that says its web viewer is too old can now fix it itself.** The built-in dashboard needs a current Android System WebView, and when the panel had an old one that screen could only tell you to go and update it, on a device whose dashboard was already down, which is the worst possible moment to be sent somewhere else. Where a known-good version is pinned for the panel and ha-paneld is permitted to install it, the screen now offers **Update the web viewer**: the panel downloads and installs that version, then ha-paneld restarts once to use it. Once ha-paneld has confirmed that it cannot repair the viewer automatically, the screen names why: a version is pinned but ha-paneld may not install it, no version is pinned for this panel, or the panel takes its web viewer from a store that will replace it more safely. The instruction you are given is one you can act on.
...
- **A panel that followed your account's default dashboard no longer disappears behind a question after an update.** Home Assistant's default dashboard is generated by a strategy at runtime, and the entity filter's safety check flags a strategy as a rule it cannot bound. The previous release re-checked every filtered dashboard after the update, so a panel that had been running happily on the account default came back holding its dashboard behind that question. A panel that was already running a filter on that dashboard now restores it only when the complete retained filter survives the post-commit catalogue update; otherwise the question remains. The recovery is deliberately narrow and the flagged rule stays visible on the Entities page. A dashboard that genuinely needs a decision still asks, and the hold screen names the panel address where the same choices can be made from another device. `GET /api/v1/dashboard/entities/sync` reports why a dashboard is held in `hold_reason`.

- **A dashboard held on a question no longer re-reads your entire entity catalogue every five minutes.** A completed scan waiting only on a decision now probes whether the dashboard changed, starting after five minutes and backing off to hourly, instead of repeatedly fetching and rewriting the catalogue. A real scan failure still retries. A changed dashboard, a late answer or changed panel settings starts one new scan, while a temporary Home Assistant outage is logged without hiding the question. `resync_suspended` in `GET /api/v1/dashboard/entities/sync` reports this state.

- **Turning the screen on when it is already on no longer dims it.** A Screen On from Home Assistant that carried no brightness was treated as a wake even on a panel that was already lit. On a panel whose profile turns the screen off by driving the brightness to zero, that wake restored the level remembered from the last screen-off, which could be long stale, floored it at a few percent, and saved the result, so a redundant Screen On from an automation dropped a bright panel to 4%. The panel now checks its own backlight first: a Screen On with no brightness on a lit panel changes nothing and simply reports the level in force, while a Screen On that names a brightness still applies it, a genuine off-to-on still restores the remembered level, and a panel found dark without having been told to sleep is still relit. [Issue #137](https://github.com/maxlyth/ha-paneld/issues/137)

- **A pinned built-in profile survives an update that edits that profile.** The profile catalog lets you pin one exact revision of a profile rather than letting the panel match one automatically, and a built-in profile's revision is the fingerprint of its file, so every release that improves a built-in profile replaces that revision with a new one. Until now a panel pinned to the old revision either kept running the retired copy it had saved for rollback and never picked up the improvement, or, when that copy was gone, came back from the update as a generic board without its buttons, LEDs, sensors or screen control until somebody pinned it again; on a panel the automatic match would have recognised anyway, the pin was quietly replaced by automatic matching instead. The panel now moves the pin to the profile's current built-in revision and checks that it comes up healthy. If it does not, the panel goes back to the revision it was running and does not try the new one again until you select it yourself or a newer one ships. Choosing the revision it was running, from the catalog or through rollback, keeps the panel there rather than moving it forward again. If the revision it was running is gone as well, the panel falls back to the generic profile and says so, and picking the profile up again from there is a choice you make: it does not re-pin itself. The log records which revision was retired and which one replaced it. A profile you imported yourself is not touched by this: if its revision is gone or can no longer be activated, the panel still refuses it and says so, rather than swapping in the built-in profile of the same name.
...
- **A rooted upgrade that cannot replace the helper now tells you why.** When the installer could not copy the new helper into place, it named the step it stopped at and nothing else, which cannot separate a partition that is mounted read-only from one that has run out of room, from a directory the panel simply refused to write. Before it changes anything, the installer now checks the exact directories it is about to write to and reports what it found there: whether that partition is writable, how much room is left, what the directory's permissions, owner and SELinux labelling are, and whether the staged helper is still present and still matches its signed checksum. A copy that fails anyway now carries the error the panel itself reported. Room is also measured where each file actually goes rather than one directory above it, so a panel too full for a `/system` replacement takes the layout that fits instead of stopping partway. A panel that genuinely cannot take the replacement is left exactly as it was, and is told which of those things was the blocker rather than to restart a helper that is not stuck. [Issue #120](https://github.com/maxlyth/ha-paneld/issues/120)
