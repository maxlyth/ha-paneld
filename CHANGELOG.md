...
- **The UI is now in six languages.** Configure → System → Interface language offers English, German, French, Italian, Spanish and Simplified Chinese. Automatic follows the Home Assistant user's language when available, then the browser or device language. Shared navigation, setting labels, help and supported runtime messages follow the same choice, with per-string English fallback where a translation is unavailable. This was all done with AI so I can't vouch for the quality.
