...
- **The ZHICAI SMT1019 can now report room temperature and humidity.** The panel carries the same GXHT30 sensor, reached the same way, as the ZX-SMT156 that ha-paneld already reads — the profile simply said otherwise, because it was written from a unit without root, and reading that sensor needs the root helper. Panels running the supplier's rooted firmware now offer Room temperature and Room humidity as opt-in sensors on the Sensors card; a panel without the helper reports nothing, exactly as before. Note the scale is inherited from the panels already using this route and has not yet been checked against this vendor's own display, so if your readings look off, `Room temperature offset` will correct them — and please say so on [issue #106](https://github.com/maxlyth/ha-paneld/issues/106) with what the panel's own test app shows at the same moment.
