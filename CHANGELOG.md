...
- **The swipe-revealed navigation bar no longer double-draws against Android's own bar on panels that have one.** On panels with a native navigation bar the reveal swipe also makes Android briefly show its hidden system bar underneath, whose buttons showed through ha-paneld's translucent bar and then vanished a couple of seconds before it. The revealed bar is now opaque on those panels, so the system bar stays masked behind it. The "Always on" bar keeps its translucent look everywhere: it sits over the dashboard permanently, and the translucency is what keeps the covered strip of the dashboard visible.
