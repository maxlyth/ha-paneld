...
- **Auto-brightness sensitivity has a useful full range and a clearer history chart.** Zero follows the learned pattern alone, higher values increase the response to measured light, existing values are migrated to preserve their effective behaviour, and the Minimum level can be set to 99%. The earlier `auto_brightness_sensitivity` API name remains accepted and is converted instead of invalidating the rest of a configuration request. Recent days stay as separate chart lines and older history is shown as a shaded range.
