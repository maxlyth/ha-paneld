...
- **Panels with no backlight device can now sleep and wake the screen.** ha-paneld's real screen-off writes the panel's Linux backlight node, and a panel that exposes none, such as a Raspberry Pi 4 running LineageOS with a third-party touchscreen, could only dim. A device profile can now select `keyevent` for `hardware.screen_off`, which sleeps and wakes Android itself with the same key presses a power button sends. This is a different state from a backlight blanked behind Android's back, so it comes with different rules: ha-paneld confirms the panel really did go to sleep instead of trusting the command's exit status, and dims to a visible floor if it cannot; it can always wake the panel again from Home Assistant, without needing root at that moment; a wake that happens at the panel itself, from a power button or from a touch on hardware that supports it, is now noticed by the panel on its own, so the dashboard comes back live rather than sitting there frozen until Home Assistant or the MQTT broker can be reached; and it refuses to sleep a panel that has a PIN, pattern or password set, because nobody is standing at a wall panel to type one. Whether a touch on the sleeping panel wakes it is up to the hardware — Android delivers touches while asleep only where the touchscreen is wired as a wake source — so a profile author has to check that on the device, and the panel's own diagnostics now say which of these applies rather than reporting a backlight-off it does not have. No shipped panel profile changes, and the existing backlight routes are untouched.

- **The built-in renderer no longer waits on the dashboard list to reopen your dashboard after a restart.** The panel remembers the dashboard that Home Assistant's own dashboard list last resolved for it and navigates straight to that on the next launch, checking the account's dashboard list and defaults in the background instead of holding the screen on "Selecting the Home Assistant dashboard" until Home Assistant answers. The short Home Assistant compatibility check still runs first, so a relaunch while Home Assistant is unreachable can pause for a few seconds before falling back to the last verified version; the open-ended dashboard-selection wait is what this removes. If the background check finds the remembered dashboard was removed or your default changed, the panel moves to the right dashboard and remembers that instead, and it never overrides a dashboard you have navigated to yourself in the meantime. An explicitly configured dashboard remains authoritative. The memory is tied to the Home Assistant address and account it was learned from and is never used for a different one, so changing server or account starts from a clean slate; a remembered dashboard belonging to a previous account becomes usable again only if that exact account is restored on the panel.

- **An unattended install can now choose the dashboard and entity filtering up front, so a slow panel never has to draw your default dashboard first.** A scripted install deliberately never asks guided setup's two questions — nobody is standing at the panel to answer them — so until now such a panel opened whatever Home Assistant treats as that account's default dashboard, which on a large account is often the heaviest page you own. `--home-dashboard` takes a dashboard or a specific view below one, or `auto` to keep following the account default, and `--entity-filter on|off` decides whether Home Assistant's state stream is limited to the entities the dashboard actually uses. Both are applied before the panel's first render, and both need ha-paneld's built-in renderer. Supplying either also answers the matching guided-setup question, so the panel does not ask it again; anything you change later on the panel takes precedence, as does an explicitly named option over a restored configuration bundle in the same command. A dashboard Home Assistant does not currently list is still saved, with a warning, so a panel can be set up before the dashboard it will show exists — but a path Home Assistant could never resolve is refused, and nothing is recorded as answered, so guided setup still asks.
...
- **The home dashboard can now be a specific view, not just a dashboard.** Home Assistant only tells ha-paneld about whole dashboards, so the picker could be pointed at a dashboard but never at one of its tabs. Guided setup and Configure now both offer a Custom choice with a path field, and a path that was configured outside the list opens there ready to edit instead of appearing as an entry that cannot be changed. If the dashboard named at the start of the path is not one this panel's Home Assistant account can see, the field says so, because the panel would otherwise quietly fall back to the account's default dashboard with nothing to explain why — but it still saves, so a panel can be set up before the dashboard it is meant to show exists.
- **Repeated Wi-Fi dropouts are now visible instead of silently forgotten.** A short blip — a few seconds off the network — should not and does not interrupt anything, but a panel that drops off Wi-Fi many times a day is telling you the Wi-Fi needs attention, and until now nobody ever learned that. The panel now counts actual losses of its Wi-Fi connection (never Home Assistant restarts or broker outages, which are not the network's fault) and keeps a rolling 24-hour total across restarts. A panel with a clean day shows nothing at all; one with episodes shows a "Wi-Fi stability" row under Runtime diagnostics, with a clear note once drops are frequent enough to mean trouble. A matching opt-in diagnostic sensor, "Wi-Fi outages (24 h)", can expose the same count to Home Assistant. The panel keeps the time each counted dropout began, so the 24-hour window is exact — no network names, hardware identifiers, signal levels or durations.
- **The panel's diagnostics now say whether the dashboard is actually up.** Until now every health surface could look perfectly well while the screen showed nothing but an error: `/api/v1/diag` had no renderer section at all, so confirming a dashboard was down meant taking a photograph of each panel. A "HA renderer" row now sits near the top of Runtime diagnostics, a matching `[renderer]` line appears in the diagnostics dump, and `GET /api/v1/status` carries a `renderer` object — all three from one observation, so they cannot disagree. They report whether Home Assistant's dashboard is connected right now, what the last admission check decided, whether it passed only on a previously confirmed Home Assistant version, whether the panel is waiting to retry by itself, and how long ago that was observed. A connection failure is named by kind — a certificate-trust failure reads as `tls_trust`, an unresolvable address as `dns` — which is the fact that turns a stuck panel into a one-line diagnosis. None of it is guesswork: a panel that has not decided anything yet says so rather than reporting health, and a panel using a Home Assistant Companion or other external renderer says plainly that ha-paneld cannot see that app's connection. Everything on these surfaces stays safe to paste into a bug report: no address, host, credential or raw error text — the transport failure is reported as a category plus the failure's class name, and the connection as its scheme alone.
...
- **Every panel screen ha-paneld shows in place of your dashboard now says so.** Startup, the Home Assistant compatibility and dashboard checks, the entity scan and entity-filter holds, the sign-in rejection notice and the panel admin screen all open with the ha-paneld name, centred, above whatever they are reporting. Previously only two of these screens identified themselves, so a message such as "Home Assistant version check rejected" — which is ha-paneld reporting its own check — read as though Home Assistant had produced it, and during a real outage that sent diagnosis down the wrong path. The name stays in a fixed position while the information beneath it changes, so progress updates no longer shift or flash the top of the screen, and it is drawn from an icon carried inside the app, so it appears even when the network or Home Assistant is unreachable. The full-screen native screens now share one layout, and every ha-paneld screen shares one light/dark rule and one colour scheme, the panel admin screen's tile colours aside — those need to sit a shade off the page behind them. A panel with a configured dashboard theme no longer shows a light screen at startup and a dark one minutes later, and the buttons on these screens now follow that theme too rather than the system's. Note that a few of these screens were previously always dark regardless of the panel's settings, so on a panel set to a light theme — or left unset on a panel whose stored preference is light — they will now correctly appear light where they did not before. These screens are sized for the smallest 480×480 panels — the compatibility screen previously had no small-screen sizing at all and could push its buttons out of reach — and they grow and scroll rather than clipping when the text size is enlarged. Paired buttons stack instead of running off the side of a narrow panel, and every control now has a visible edge against the page behind it. The panel admin screen and the two pages drawn by the browser are laid out by their own rules and carry the name without being part of that shared layout.

- **The installer now tells you when the panel is in a different time zone from you.** A panel still set to the zone its factory image shipped with will timestamp its logs and run its schedules in that zone, and nothing on the panel ever mentions it, so this usually surfaces much later as something happening at the wrong hour. Before anything is installed, the installer states the panel's time zone next to the one the computer you are running it from is set to, and warns you when the two are different zones. It changes neither clock and never refuses an installation, because pointing a panel at another zone on purpose is a reasonable thing to want. The two are compared by zone name rather than by the offset they read at that moment, so a pair that agrees today and parts when one of them moves for summer time is still reported, and two names for a single zone, such as `Asia/Calcutta` and `Asia/Kolkata`, are recognised as one zone rather than flagged. This is a remark and nothing more, so it stays quiet unless it can actually tell: if either time zone cannot be read, or your computer's time-zone database does not carry one of the names, the installer says nothing at all rather than raising a question you would then have to dismiss.

### Changed

- **The auto-brightness Sensitivity control now uses its whole range.** It is the percentage of a difference from the learned pattern that gets applied once the controller decides the difference is real; 0% uses the learned pattern alone. The old scale put neutral at 50 and spent everything above it over-reacting past the measured light, so half the control did nothing useful and the low end stepped too coarsely to tune. Existing panels keep the same response, rescaled on upgrade, except any panel set above the old 50, which lands on full response. Minimum level now goes up to 99%.

### Fixed

- **A panel that reports a successful reboot but never reboots is now caught and retried.** ha-paneld asks the root helper to reboot, and the helper's first mechanism is Android's `svc power reboot`. On some builds that command reports success without doing anything, which made the helper's own fallback unreachable and told Home Assistant the panel was going down when it was not. The helper no longer treats a command's exit status as proof: each mechanism gets a bounded window, and if the panel is plainly still running when that window ends it goes on to the next one. A command that hangs rather than answering is cut off at the same boundary, so one wedged reboot request can no longer hold up every mechanism behind it — or, on a busy panel, sit there occupying one of the helper's connections indefinitely. When every mechanism has been tried and the panel is still up, that now reaches the app as a failure rather than as a reboot, so ha-paneld's own root route gets its turn — it runs from a different environment, which is exactly what made the difference on the panel where this was reported. A reboot that works behaves as before.
- **The ambient-light history chart no longer depends on a browser-specific effect.** Previous days were softened using a canvas filter that some browsers apply and others do not, so the same history could look substantially different depending on where you opened the Configure page — and because the effect was invisible in one of them, its strength had been raised until it smeared older days into a haze wherever it did apply. The softening is now drawn directly, using only the basic drawing operations browsers implement uniformly, so it no longer rests on optional support. The chart also draws today, yesterday and two days ago in full and shows the rest of the week as a shaded band between the lowest and highest reading recorded at each time of day — nine lines instead of twenty-one, which makes recent days far easier to tell apart. Your stored ambient-light history, the learned pattern and the brightness the panel chooses are all unchanged; only the picture is different.
- **Renderer error screens now recover on their own when the cause clears.** Almost every blocked compatibility and dashboard-selection screen previously waited for someone to press Retry (only the dashboard-list check retried by itself), so a Home Assistant outage could leave a wall panel parked on an error screen after the server recovered — nobody is standing at a wall panel to press the button. A screen that could not finish its check — Home Assistant unreachable, its dashboard list unreadable, the sign-in page unavailable, or the secure bridge handshake missed — now retries by itself, starting within seconds and spacing out to roughly five-minute intervals (each delay is varied slightly so many panels do not retry in unison), with a visible countdown to the next attempt. A screen reporting something only the server can change, where nothing would ever tell the panel — an account that currently reaches no dashboard, or a version Home Assistant reported unusably — is re-checked quietly at that same five-minute spacing. Three screens deliberately do not run a timer, because a timer would change nothing: a rejected or missing Home Assistant login (fixing the connection restarts the check immediately), a Home Assistant older than this renderer supports, and an Android System WebView that cannot provide the secure bridge. Every screen keeps its Retry button, which acts at once and restarts the fast retries.
- **A Home Assistant server that cannot be reached is no longer reported as "could not authenticate".** When the panel's access-token refresh failed in transport — a broken TLS certificate chain, a DNS failure, a timeout — the compatibility check misreported it as an authentication rejection, pointing diagnosis at credentials instead of at the connection. The screen now names the actual transport failure, and a panel that has already confirmed a supported Home Assistant version no longer stops at the version check when it cannot reach the server — it carries on with startup using that confirmed version, though later steps still need Home Assistant to be reachable.
- **A panel whose network breaks IPv6 toward Home Assistant no longer gets stuck unable to load its dashboard.** The panel's Home Assistant WebSocket connections — the ones that read your dashboard list and entities before the dashboard opens — previously connected to a single resolved address, so a hostname whose IPv6 address was silently unreachable (common on bridged or remote network segments and some VPNs) could hang forever even though IPv4 worked perfectly, leaving the panel on an error screen while every other device on the network coped. These connections now try every published address with a bounded per-attempt timeout and race IPv6 against IPv4 the way browsers do, and they honor the existing address-family setting (`Automatic` / `Prefer IPv4` / `Force IPv4`) rather than leaving it MQTT-only. Note for upgrades: if you had set this to a non-default value for your broker, it now also constrains the panel's Home Assistant WebSocket connections; the setting is relabeled "MQTT + Home Assistant WebSocket address family" to match. Plain HTTPS reads are unaffected — they already try every published address.
...
- **The panel now explains it when Home Assistant is the thing that went away.** A Home Assistant restart used to leave the dashboard looking frozen with nothing to say why. Panels using the built-in renderer now show a native notice naming Home Assistant as the cause — large enough to read across a room — and clear it once Home Assistant proves it is back; the panel's own web pages report the same status for every renderer. An ordinary network drop keeps the existing connection-recovery behaviour and is never described as a Home Assistant shutdown. This also works with a non-administrator Home Assistant account when the panel's MQTT connection is configured: Home Assistant only permits its lifecycle events to be watched by an administrator, so on a non-administrator account the panel reads Home Assistant's MQTT availability messages instead, and the notice says Home Assistant has gone offline rather than claiming a deliberate shutdown.

- **The standalone root-helper installer now says what a panel actually answered instead of giving up anonymously.** On a rooted panel, `helper/install-daemon.sh` asks what state the panel is in and matches the answer exactly, so anything it does not recognise stopped the run at "could not determine the root-helper recovery state" — with no way to tell what the panel had said, and advice to restore adb and root that were both already working. It now reports the answer it could not place, including whether that answer arrived carrying invisible characters, so a report can name the real cause. Replies are also read identically whichever kind of connection a panel's adb serves; a panel sending terminal-style line endings previously matched nothing, which would then have hidden a writable system partition too and left the helper installed as no boot service at all. Nothing was loosened — an unrecognised state still stops the run before any live helper file is replaced. And a panel that genuinely has no writable system partition and no supported root manager still says exactly that, rather than being reported the same way as a panel that simply could not be understood.

- **A rooted panel that stops with "read-only /system" now gets told the thing that usually fixes it.** Installing the root helper needs somewhere it can be registered to start at boot, and on Android 10 and newer that place is guarded by dm-verity — so a genuinely rooted panel can still refuse the write. The message only ever offered the other route, installing a root manager such as Magisk, KernelSU or APatch, which is a lot of work to go and do when the panel is a userdebug build with an unlocked bootloader and just needs verity turned off once. Both the installer and the standalone helper script now show that one-time sequence first, warn that it reboots the panel — so you can be standing at it, which matters if the panel has a PIN — and keep the root-manager route as what to do if the remount is still refused afterwards.

- **Installing a locally built APK no longer rolls the root helper back for looking like the wrong helper.** After replacing the helper on a rooted panel, the installer asks the newly started daemon to state which build it is, and refuses to go on to the app itself unless the answer matches. For a locally built APK it worked out the expected answer from the helper sources sitting next to the installer script rather than from the helper inside the APK it was installing, so the two only agreed while both came from the same checkout at the same moment. Install an APK built from anywhere else and a perfectly good helper was restored to the previous version and reported as having failed its build-identity check, with the app left uninstalled. The expected answer now comes from the helper embedded in the APK being installed, which is the helper that will actually be running. Nothing is loosened: a daemon that answers with any other build still stops the run and restores the previous helper before the app is touched, and an official release is still checked against the identity recorded in its signed installer rather than against the bytes it served.

- **The installer no longer blames the app when the fault is a missing Java runtime.** Before installing, the provisioner proves the downloaded app is genuine by checking its signed checksum against a key built into the installer. It then ran a second, optional structural inspection using `apksigner` — and `apksigner` is a wrapper that needs Java, so on a machine without one it fails the moment it starts. The installer read that as "this app is not properly signed", stopped, and told you to doubt a download it had just proved authentic one line earlier. It now treats a tool that cannot run the same way it has always treated one that is not installed: it says so, names the tool and the reason, and continues, because the signed checksum is what establishes authenticity. Verification is not weakened anywhere it was doing real work — a locally built APK, or any run asking for the official release signer, still stops if the signer cannot be checked.

- **A panel you rooted yourself now gets the root helper it can plainly run.** ha-paneld carries the exact helper for each release and can install it itself on a panel where the app has root — but it first checked what the device profile *said* about root, and profiles are written against whatever firmware their author had. Flash a firmware that adds root and the profile still describes the stock one, so the panel declared it had no root, and the check stopped there without ever asking the panel. It now asks: root is established by trying it, not by looking it up. Panels that genuinely have no root are unaffected and are still skipped, which is the same protection as before — a panel whose only privileged path is the helper must not replace the helper underneath itself, and that is precisely the case where trying root fails.

- **The ZHICAI SMT1019 now switches its screen fully off rather than dimming it to zero.** The panel's backlight can be powered down properly through the root helper, which is genuinely dark and lets the dashboard stop rendering behind it; the profile had been asking for the no-root route that only sets brightness to zero, which some panels visibly clamp. On stock firmware with neither the helper nor direct root, ha-paneld falls back to the same brightness-zero behaviour as before; a helper-less rooted build can use direct root. This costs the stock configuration one refused helper connection attempt at screen-off and nothing else.

- **The ZHICAI SMT1019 can now use its proximity sensor.** The panel has a time-of-flight distance sensor that nothing could previously reach, because its driver stays completely silent until something starts it — so the sensor looked absent even though it was fitted. ha-paneld now starts it the way its own vendor does and reads the distance back, feeding the same wake-on-approach learning every other proximity source uses; the panel works out which direction means *near* by watching itself, so nothing rests on a number picked in advance. It needs the root helper, so this applies to panels running the supplier's rooted firmware. Being straight about it: this is built from the vendor's driver sources and has not been run on the hardware yet, so if you have one of these panels, please say how it behaves on [issue #106](https://github.com/maxlyth/ha-paneld/issues/106).

- **The ZHICAI SMT1019 can now report room temperature and humidity.** The panel carries the same GXHT30 sensor, reached the same way, as the ZX-SMT156 that ha-paneld already reads — the profile simply said otherwise, because it was written from a unit without a privileged read route. Room temperature and Room humidity are enabled by default on the Sensors card; without a verified root helper or approved Shizuku shell they publish no value. The raw encoding is confirmed on this hardware: the driver bounds and live readings 2479/5704 are consistent with hundredths, exactly as ha-paneld reads them. Physical accuracy has not been compared with the vendor display or a reference sensor; `Room temperature offset` can correct a constant bias from the panel's own warmth, not multiplicative scaling or humidity calibration.

- **Entities you pinned or excluded by hand now survive a dashboard change.** Pointing a panel at a different dashboard re-learns the entities it discovers on its own, which is intended — a different dashboard shows different things. It also used to discard the entities you had pinned or excluded yourself, so deliberate work disappeared the moment you switched dashboard, and a lost exclusion was the quieter failure of the two: a missing pin shows up as a card that stops updating, while a missing exclusion silently starts subscribing again to something you had removed on purpose. Those choices are yours, so they are now held for the whole Home Assistant instance rather than tied to one dashboard, and ha-paneld will not remove one on its own. A pin the current dashboard no longer uses now appears under "Stale or noisy entities" for you to remove or keep, instead of being dropped silently or kept invisibly.
