...
- **The ZHICAI SMT1019 can now report room temperature and humidity.** The panel carries the same GXHT30 sensor, reached the same way, as the ZX-SMT156 that ha-paneld already reads — the profile simply said otherwise, because it was written from a unit without a privileged read route. Room temperature and Room humidity are enabled by default on the Sensors card; without a verified root helper or approved Shizuku shell they publish no value. The inherited scale has not yet been checked against this vendor's display: `Room temperature offset` can correct a constant temperature bias, but scaling and humidity still need comparison with the vendor test app. Please report that comparison on [issue #106](https://github.com/maxlyth/ha-paneld/issues/106).
