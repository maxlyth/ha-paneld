...
- **The panel now measures its own network path to Home Assistant and says when that path, not the panel, is what is slow.** A poorly placed access point or a congested 2.4 GHz channel can add seconds to every tap while Android still reports a perfectly good Wi-Fi connection, and until now the only evidence was a dashboard that felt broken and a panel that got the blame. The app already keeps one authenticated Home Assistant WebSocket open whenever it has a reason to (the built-in renderer watching for a Home Assistant restart, a Home Assistant ambient light or presence entity, or a registry watch); it now sends a tiny ping on that socket every ten seconds and measures the round trip on the path your dashboard actually uses. There is no ICMP ping, no background process and no root involved. Over a rolling five-minute window the panel keeps the median, the 95th percentile, the worst round trip, the jitter and how many probes went unanswered, and classifies the path: a warning when the 95th percentile passes 100 ms, and severe when it passes 1,000 ms, when two probes in a row go unanswered, or when more than 5% of the window's probes are lost. One lone slow reply or one lone miss is shown as evidence but never raises an alarm, and the notice ages out by itself as the bad samples leave the window. Home Assistant closing the connection, refusing it while it restarts, rejecting the sign-in or returning an error is counted separately and never reported as packet loss, so a restarting server does not get blamed on your Wi-Fi. A ping that goes unanswered while Home Assistant is still sending anything at all, including a late reply to an earlier ping, is treated the same way: the server is busy, the socket is plainly alive, and the panel keeps it open rather than reconnecting. The Dashboard tab shows the warning at the top of the page in the same style as its other severe warnings, adds an **HA network path** row to Runtime diagnostics, and Dashboard responsiveness now names the network path as the likely cause before it blames entity filtering, card JavaScript or panel CPU. On the panel itself a small **HA network slow** chip appears in a corner of the dashboard while the warning stands; it turns red when the verdict is severe, never covers the dashboard, and passes every touch straight through. The `/api/v1/diag` report and `/api/v1/status` carry the same verdict as terse numbers only, never the Home Assistant address, the network name or individual samples. The path is measured only while the panel actually holds a signed-in Home Assistant connection, so a panel that is still connecting, one that has no credentials, or one whose connection has given up reports it as not measured rather than as healthy.
