...
- **Panels with no backlight device can now sleep and wake the screen.** ha-paneld's real screen-off writes the panel's Linux backlight node, and a panel that exposes none, such as a Raspberry Pi 4 running LineageOS with a third-party touchscreen, could only dim. A device profile can now select `keyevent` for `hardware.screen_off`, which sleeps and wakes Android itself with the same key presses a power button sends. This is a different state from a backlight blanked behind Android's back, so it comes with different rules: ha-paneld confirms the panel really did go to sleep instead of trusting the command's exit status, and dims to a visible floor if it cannot; it can always wake the panel again from Home Assistant, without needing root at that moment; and it refuses to sleep a panel that has a PIN, pattern or password set, because nobody is standing at a wall panel to type one. Whether a touch on the sleeping panel wakes it is up to the hardware — Android delivers touches while asleep only where the touchscreen is wired as a wake source — so a profile author has to check that on the device, and the panel's own diagnostics now say which of these applies rather than reporting a backlight-off it does not have. No shipped panel profile changes, and the existing backlight routes are untouched.
