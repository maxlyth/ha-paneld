...
- **Experimental: a panel with a camera can serve a snapshot to Home Assistant, off by default and on trial.** On a device profile that declares a camera, Configure gains a **Camera** switch plus resolution, frame-rate and bitrate ceilings, and `GET /api/v1/camera/snapshot.jpg` returns a JPEG that Home Assistant's Generic Camera can poll as a still image. The camera opens only while a client is actually asking for a frame and closes again afterwards, so an idle panel spends nothing on it. Whenever the camera is open the panel shows a red light in the top corner that page content cannot cover, on every Android version, and if that light cannot be shown the camera stays closed; while the screen is off the status LED carries the indication instead. `GET /api/v1/status` gains a `camera` object and the diagnostics dump a `[camera]` line, so a camera that has stopped working is visible rather than silent. No bundled profile turns this on yet, the app asks for the Android camera permission only after the switch is on, and no microphone access is declared because the trial is video only. A live stream and Home Assistant entities follow in later builds.
### Added
