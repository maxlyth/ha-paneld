...
- **The ZHICAI SMT1019 can now report room temperature and humidity.** The panel carries the same GXHT30 sensor, reached the same way, as the ZX-SMT156 that ha-paneld already reads — the profile simply said otherwise, because it was written from a unit without a privileged read route. Room temperature and Room humidity are enabled by default on the Sensors card; without a verified root helper or approved Shizuku shell they publish no value. The scale is confirmed on this hardware: the driver publishes its own range as -40.00 to 125.00 °C and 0.00 to 100.00 %, and a panel reading 2479 and 5704 was 24.79 °C and 57.04 % — hundredths, exactly as ha-paneld already reads them. `Room temperature offset` remains available for the small constant bias a panel's own warmth adds.
