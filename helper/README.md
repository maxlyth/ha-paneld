...
| `KEYEVENT SLEEP` / `KEYEVENT WAKEUP` | Android screen power for panels with no `/sys/class/backlight` device, by injecting the named key. Named keys only — there is no numeric form, so no caller or profile can select another keycode. `input` is an app_process wrapper and can wedge instead of failing, so the injection runs under a deadline and is killed at it rather than holding the connection | `OK` (the request ran) / `ERR` |
