...
docker run --rm -v "$HOST_WORK":/work -w /work "$IMAGE" bash -c '
  ...
  "$TC/armv7a-linux-androideabi30-clang" -O2 -s -Ihelper/src -o helper/dist/armeabi-v7a/hapaneld-helper $SRCS
  "$TC/aarch64-linux-android30-clang"    -O2 -s -Ihelper/src -o helper/dist/arm64-v8a/hapaneld-helper  $SRCS
'
